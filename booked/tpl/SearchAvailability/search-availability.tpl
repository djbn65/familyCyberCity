    ...
    <form role="form" name="searchForm" id="searchForm" method="post"
        ...
        <div class="form-group col-xs-12 col-sm-3">
            <div class="checkbox">
                ...
                <label for="anyResource" style="color:#888">{translate key=AnyResource}</label>
            </div>
        </div>
        ...
        <div class="form-group col-xs-12 col-sm-9">
            <div class="btn-group margin-bottom-15" data-toggle="buttons">
                <label class="btn btn-default active">
                    ...
                    <span class="hidden-xs" style="color:#2e45f7">{translate key=Today},</span>
                    <span style="color:#2e45f7"> {format_date date=$Today key=calendar_dates}</span>
                </label>
                <label class="btn btn-default">
                    <input type="radio" id="tomorrow" value="tomorrow" {formname key=AVAILABILITY_RANGE} />
                    <span class="hidden-xs" style="color:#2e45f7">{translate key=Tomorrow},</span>
                    <span style="color:#2e45f7"> {format_date date=$Tomorrow key=calendar_dates}</span>
                </label>
                <label class="btn btn-default">
                    <input type="radio" id="thisweek" value="thisweek" {formname key=AVAILABILITY_RANGE} />
                    <span class="hidden-xs" style="color:#2e45f7">{translate key=ThisWeek}</span>
                    <span class="visible-xs" style="color:#2e45f7">{translate key=Week}</span>
                </label>
                <label class="btn btn-default">
                    <input type="radio" id="daterange" value="daterange" {formname key=AVAILABILITY_RANGE} />
                    <i class="fa fa-calendar"></i><span class="hidden-xs" style="color:#2e45f7"> {translate key=DateRange}</span>
                </label>
            </div>
            <div class="">
                ...
                <a href="#" data-toggle="collapse" data-target="#advancedSearchOptions" style="color:#888">{translate key=MoreOptions}</a>
            </div>
            <div class="clearfix"></div>
